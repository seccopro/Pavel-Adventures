...
func cast(magic : String, state: State ):
...
func cast_magic_orb():
	if(not magic_orb_is_on_cooldown):	#should not need this check
...
func cast_dark_sphere():
	if(not dark_sphere_is_on_cooldown):	#should not need this check
...
func _on_dark_sphere_cooldown_timeout():
	print("dark sphere off cooldown")
	dark_sphere_is_on_cooldown = false

func _on_magic_orb_cooldown_timeout():
